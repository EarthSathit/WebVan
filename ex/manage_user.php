...
switch($action){
    case "select":
    if (isset($_GET['phone'])) {
      $stmt = $str_exe->readAll("users where phone = '".$_GET['phone']."'");
    } else {
    ...
    "id_card, initial_id, name, lastname, email, phone, password, user_type_id",
    "NULL, NULL, '$name', '$lastname', '$email', '$phone', NULL, NULL");
    if($strSQL){
